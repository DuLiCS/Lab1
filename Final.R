...
Edmonton_climate_2020 <- read.csv("en_climate_daily_AB_3012209_2020_P1D.csv")
Edmonton_climate <- rbind(Edmonton_climate_2020,Edmonton_climate)

Halifax_climate <- read.csv("Halifax_climate_daily_Nova Scotia_P1D.csv")
Halifax_climate_2020 <- read.csv("en_climate_daily_NS_8202251_2020_P1D.csv")
Halifax_climate <- rbind(Halifax_climate_2020, Halifax_climate)

Montreal_climate <- read.csv("Montreal_climate_daily_Quebec_P1D.csv")
Montreal_climate_2020 <- read.csv("en_climate_daily_QC_7025251_2020_P1D.csv")
Montreal_climate <- rbind(Montreal_climate_2020, Montreal_climate)

StJohn_climate <- read.csv("St.John's_climate_daily_Newfounland and Labrador_P1D.csv")
StJohn_climate_2020 <- read.csv("en_climate_daily_NL_8403505_2020_P1D.csv")
StJohn_climate <- rbind(StJohn_climate_2020, StJohn_climate)

Toronto_climate <- read.csv("Toronto_climate_daily_Ontario_P1D.csv")
Toronto_climate_2020 <- read.csv("en_climate_daily_ON_6158355_2020_P1D.csv")
Toronto_climate<- rbind(Toronto_climate_2020, Toronto_climate)


Vancouver_climate <- read.csv("Vancouver_climate_daily_British Columbia_P1D.csv")
Vancouver_climate_2020 <- read.csv("en_climate_daily_BC_1108395_2020_P1D.csv")
Vancouver_climate <- rbind(Vancouver_climate_2020,Vancouver_climate)
...
for (y in 2020:2021)
{for (m in 1:12)
{temp_set <- subset(Edmonton_merge, Month == m & Year == y) 
print(m)
...
p <- ggplot(dd, aes(x = date_stamp))+
  geom_point(aes(y = mean_temp, color = "mean temperature"))+
  geom_point(aes(y = (num_confirmed - 19000)/600, color = "num of confirmed cases"))+
...
p <- ggplot(Edmonton_merge, aes(x = date))+
  geom_point(aes(y = Mean.Temp...C., color = "mean temperature"))+
  geom_point(aes(y = (numconf - 19000)/600, color = "num of confirmed cases"))+
  scale_y_continuous(sec.axis = sec_axis(~.*600 + 19000, name = "num of confirmed cases"))+
  scale_color_manual(values = c("red","green"))+
  labs(title = "Edmonton")+
  theme(plot.title = element_text(hjust = 0.5),
        axis.title.y = element_text(size = 12))
ggsave(p,filename = "pEdmontonAll.png",width = 10,height = 10,limitsize = F)
...
p <- ggplot(dd, aes(x = date_stamp))+
  ...
  geom_point(aes(y = (num_confirmed - b)/a, color = "num of confirmed cases"))+
...
for (y in 2020:2021)
{for (m in 1:12)
{temp_set <- subset(Halifax_merge, Month == m & Year == y) 
print(m)
early_month <- subset(temp_set, Day>=1 & Day<15)
early_month <- subset(early_month, !is.na(Mean.Temp...C.))
late_month <- subset(temp_set, Day>=15 & Day <=31)
...
for (y in 2020:2021)
{for (m in 1:12)
{temp_set <- subset(Montreal_merge, Month == m & Year == y) 
print(m)
early_month <- subset(temp_set, Day>=1 & Day<15)
early_month <- subset(early_month, !is.na(Mean.Temp...C.))
late_month <- subset(temp_set, Day>=15 & Day <=31)
...
for (y in 2020:2021)
{for (m in 1:12)
{temp_set <- subset(StJohn_merge, Month == m & Year == y) 
print(m)
early_month <- subset(temp_set, Day>=1 & Day<15)
early_month <- subset(early_month, !is.na(Mean.Temp...C.))
late_month <- subset(temp_set, Day>=15 & Day <=31)
...
for (y in 2020:2021)
{for (m in 1:12)
{temp_set <- subset(Toronto_merge, Month == m & Year==y) 
print(m)
early_month <- subset(temp_set, Day>=1 & Day<15)
early_month <- subset(early_month, !is.na(Mean.Temp...C.))
late_month <- subset(temp_set, Day>=15 & Day <=31)
...
for (y in 2020:2021)
{for (m in 1:12)
{temp_set <- subset(Vancouver_merge, Month == m & Year == y) 
print(m)
early_month <- subset(temp_set, Day>=1 & Day<15)
early_month <- subset(early_month, !is.na(Mean.Temp...C.))
late_month <- subset(temp_set, Day>=15 & Day <=31)
